...
export const POST=route(async request=>{
  ...
  let tx;try{tx=verifyPreparedLaunch(input.transaction,prepared.message,prepared.mintSignature?{publicKey:input.mint,signature:prepared.mintSignature}:undefined);}catch(error){
    ...
    throw new HttpError(403,'Signed transaction does not match the prepared launch.');
  }
