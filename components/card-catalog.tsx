...
import { CardCatalogSkeleton, Skeleton } from './ui/skeleton';
...
export function CardCatalog({embedded=false}:{embedded?:boolean}) {
  ...
  return <div className={`card-catalog ${embedded?'catalog-embedded':''} ${brand?'has-selection':''} stage-${step}`}>
    ...
    {history?<CardOrders/>:<>
    <div className="catalog-heading"><div><h2>{brand?(step==='amount'?'Choose your amount':step==='checkout'?'Checkout':'Your purchase'):'Pick a gift card'}</h2></div>{brand&&<button className="text-button" onClick={()=>{if(step==='checkout'){setStep('amount');}else{setBrand(null);setLoading(false);setStep('amount');}}}><ArrowLeft size={16}/> {step==='checkout'?'Amount':'Cards'}</button>}</div>
    ...
    {!brand?<>
      ...
      {loading?<CardCatalogSkeleton/>:<div className="catalog-grid" role="region" aria-label="Available gift cards" tabIndex={0}>{matching.map(b=><button className="catalog-card" key={b.id} onClick={()=>{setBrand(b);setStep('amount');}}>
        ...
        <strong>{b.name}</strong><span className="catalog-card-price"><span className="catalog-card-price-copy">{b.minimum?<span className="catalog-card-price-primary"><span className="catalog-card-price-label">From</span><strong className="catalog-card-price-value">{b.minimum}</strong></span>:<strong className="catalog-card-price-value">View amounts</strong>}{b.country==='US'&&b.minimum&&/^\$[\d,.]+$/.test(b.minimum)&&<SolEquivalent amount={b.minimum.replace(/[$,]/g,'')}/>}</span><ArrowUpRight size={14}/></span>
      </button>)}</div>}
      ...
      {loading&&!product&&<div className="catalog-selection-skeleton" aria-label="Loading card details" aria-busy="true"><div className="selected-perk"><Skeleton className="skeleton-selected-art"/><div><Skeleton className="skeleton-selected-title"/><Skeleton className="skeleton-selected-copy"/></div></div><Skeleton className="skeleton-amount-field"/><Skeleton className="skeleton-range-copy"/><Skeleton className="skeleton-checkout-button"/></div>}
      {product&&<>
        ...
        {!product.fixed ? <label className="catalog-amount"><span className="currency-input"><span className="currency-prefix" aria-hidden="true">{new Intl.NumberFormat('en-US',{style:'currency',currency:product.currency,currencyDisplay:'narrowSymbol'}).formatToParts(0).find(part=>part.type==='currency')?.value??product.currency}</span><input aria-label={`Amount in ${product.currency}`} type="number" min={product.min} max={product.max} step={product.step} value={amount} onChange={e=>{setAmount(e.target.value);}}/><SolEquivalent amount={amount} currency={product.currency}/></span></label> : <SolEquivalent amount={amount} currency={product.currency}/>}
        {!product.fixed&&<p className="catalog-count">{product.min}–{product.max} {product.currency}</p>}
